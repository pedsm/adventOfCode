...
func part1(tree FileSystemItem) {
	sizeOfDirsBelow := []int64{}

	search(&tree, &sizeOfDirsBelow)
	fmt.Println(sizeOfDirsBelow)

	result := int64(0)
	for _, num := range sizeOfDirsBelow {
		result += num
	}
	fmt.Println(result)
}

func search(tree *FileSystemItem, sizeOfDirsBelow *[]int64) {
	if tree.size == 0 {
		dirSize := size(*tree)
		if dirSize < 100000 {
			*sizeOfDirsBelow = append(*sizeOfDirsBelow, dirSize)
		}
	}

	for i, _ := range tree.contents {
		search(&tree.contents[i], sizeOfDirsBelow)
	}
}
